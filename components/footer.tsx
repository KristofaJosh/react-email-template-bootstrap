...
const getUnsubscribeHref = ({ email }: { email: string }) => {
  ...
  return `mailto:support@haustidy.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`
}

export const Footer = ({ email }: { email: string }) => {
  const { baseAssetUrl, haustidyLogo } = useEmailVariables()
    ...
    <Section className={'mt-8'}>
      ...
      <Row>
        <Text className="mb-8 text-base text-gray-600">
          ...
          support@haustidy.com.
          <br />
          <span className="mt-4 block">
            &copy; {new Date().getFullYear()} Haustidy. All rights reserved.
          </span>
        </Text>
      </Row>
